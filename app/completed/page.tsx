...
import { Eye, Heart, Search, Play, CheckCircle, BookOpen } from "lucide-react"
...
export default function CompletedPage() {
  ...
  useEffect(() => {
    const fetchCompletedWorks = async () => {
      try {
        setLoading(true)
        // 완결 상태가 있는 작품들을 가져오거나, 일정 분량 이상의 작품들을 완결작으로 간주
        ...
        // 완결작 기준: 만화는 10화 이상, 소설은 20페이지 이상
        const completedWorks = allWorks.filter((work) => {
          if (work.status === "completed") return true
          if (work.type === "comic" && (work.comicContent?.length || 0) >= 10) return true
          if (work.type === "novel" && (work.novelContent?.length || 0) >= 20) return true
          return false
        })

        setWorks(completedWorks)
      } catch (error) {
        console.error("Completed works fetch error:", error)
      } finally {
        setLoading(false)
      }
    }
      ...
      <main className="container px-4 py-8">
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h1 className="text-3xl font-bold">완결작</h1>
          </div>
          <p className="text-muted-foreground">완결된 작품들을 한번에 몰아보세요</p>
        </div>
        ...
        <div className="mb-8 space-y-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="text-sm text-muted-foreground flex items-center">총 {filteredWorks.length}개 완결작</div>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-semibold mb-2">완결작이 없습니다</h3>
            <p className="text-muted-foreground">완결된 작품이 업로드되면 여기에 표시됩니다.</p>
          </div>
        ) : (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
                ...
                <Card className="group cursor-pointer hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
                  <CardContent className="p-0">
                    <div className="relative overflow-hidden rounded-t-lg">
                      ...
                      />
                      <Badge className="absolute top-2 left-2 bg-green-500 hover:bg-green-600 text-white">완결</Badge>
                      <Badge
                    ...
                    <div className="p-4">
                      ...
                      <div className="flex items-center gap-1 text-xs text-muted-foreground">
                        ...
                        <span>
                          {work.type === "comic"
                            ? `${work.comicContent?.length || 0}화 완결`
                            : `${work.novelContent?.length || 0}페이지 완결`}
                        </span>
                      </div>
                    </div>
                  </CardContent>
