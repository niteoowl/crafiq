...
export default function NovelsPage() {
  ...
  useEffect(() => {
    const fetchNovels = async () => {
      try {
        setLoading(true)
        const q = query(collection(db, "works"), where("type", "==", "novel"), orderBy("createdAt", "desc"))
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="container px-4 py-8">
        <div className="mb-8">
          <h1 className="text-3xl font-bold mb-2">소설</h1>
          <p className="text-muted-foreground">다양한 장르의 소설을 만나보세요</p>
        </div>
        ...
        <div className="mb-8 space-y-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            ...
            <div className="text-sm text-muted-foreground flex items-center">총 {filteredWorks.length}개 소설</div>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            <p className="text-muted-foreground">조건에 맞는 소설이 없습니다.</p>
          </div>
        ) : (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
